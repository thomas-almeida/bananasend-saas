      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {
          prices && prices.length > 0 ? (
            prices.map((price: SubscriptionPlan, index: number) => (
              <div
                key={price._id}
                className="flex flex-col items-center justify-center border border-slate-200 p-4 rounded-xl bg-white shadow-2xl h-[480px] gap-2 hover:scale-102 transition-all cursor-pointer"
              >
                <p className="text-xs">
                  {
                    index === 0 ? (
